...
API_KEY = "eyJ0eXAiOiJKV1QiLCJhbGciOiJIUzUxMiIsImtpZCI6IjI4YTMxOGY3LTAwMDAtYTFlYi03ZmExLTJjNzQzM2M2Y2NhNSJ9.eyJpc3MiOiJzdXBlcmNlbGwiLCJhdWQiOiJzdXBlcmNlbGw6Z2FtZWFwaSIsImp0aSI6Ijk0N2RkNmFlLTNmNDUtNGJlMS04NzU1LTFlYmJmNmJiMDdjZiIsImlhdCI6MTc1Njg3MTQ5MSwic3ViIjoiZGV2ZWxvcGVyL2ZjNTE2YWY0LTA4YzUtY TUwYS1iNjA1LTA0NWJiN2Y2MWYxNyIsInNjb3BlcyI6WyJjbGFzaCJdLCJsaW1pdHMiOlt7InRpZXIiOiJkZXZlbG9wZXIvc2lsdmVyIiwidHlwZSI6InRocm90dGxpbmcifSx7ImNpZHJzIjpbIjIwMS4xNzguMTk0LjM3Il0sInR5cGUiOiJjbGllbnQifV19.uctnTjCp3XMVTSmxCOM4f_2GiwHpn33WvhVMPJ5JksIpns9YyaSe9_bTJVwr5tKGH4Mc8nTTRm6ExaBLQRIGmQ"
API_BASE_URL = "https://api.clashofclans.com/v1"

# Cache para datos de clanes
clan_cache = {}
daily_stats_cache = {}
last_snapshot_data = {}  # NUEVO: Cache para snapshots
last_update = None
...
DONATIONS_FILE = "daily_donations.json"
BACKUP_FILE = "donations_backup.json"
SNAPSHOT_FILE = "donations_snapshot.json"  # NUEVO: Archivo de snapshots

def save_snapshot():
    """NUEVO: Guarda un snapshot de las donaciones totales actuales"""
...
def load_snapshot():
    """NUEVO: Carga el último snapshot guardado"""
    global last_snapshot_data
...
def calculate_daily_from_snapshot():
    """NUEVO: Calcula donaciones diarias usando snapshots"""
        ...
        for clan_tag in clans.keys():
            try:
                ...
                for member in clan_data['memberList']:
                    ...
                    # NUEVO: También resetear el snapshot
                    last_snapshot_data[cache_key] = {
        ...
        for clan_tag in clans.keys():
            try:
                ...
                for member in clan_data['memberList']:
                    ...
                    # NUEVO: También resetear snapshot
                    last_snapshot_data[cache_key] = {
                        'total_donations': current_donations,
                        'total_received': current_received,
                        'snapshot_time': now_argentina.isoformat(),
...
def enhanced_auto_update_worker():
    """NUEVO: Worker mejorado que guarda snapshots en cada actualización"""
    ...
    while True:
        try:
            ...
            # NUEVO: Guardar snapshot en cada actualización
            try:
                save_snapshot()
...
def enhanced_startup():
    """NUEVO: Inicialización mejorada con sistema de snapshots"""
...
def main():
    ...
    # NUEVO: Inicialización mejorada con snapshots
    enhanced_startup()
    ...
    # NUEVO: Hilo mejorado para actualizaciones con snapshots
    ...
    try:
        ...
        with socketserver.TCPServer(("", PORT), RequestHandler) as httpd:
            ...
            print("\n🎯 El servidor está listo para recibir conexiones")
            print("📱 Ahora puedes abrir la página web y las donaciones se guardarán automáticamente\n")
